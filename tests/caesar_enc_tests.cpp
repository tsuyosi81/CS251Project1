...
TEST_P(CaesarEnc_FuncCommand, Case) {
  ...
  ASSERT_THAT(actual_output, HasSubstr(expectedEnc))
...
const string TEXT_PROMPT = "Enter the text to encrypt:";
const string NUMBER_PROMPT = "Enter the number of characters to rotate by:";
...
TEST_F(CaesarEnc_MainCommand, Main) {
  ...
  ASSERT_THAT(actual_output, HasSubstr(TEXT_PROMPT))
      << "\nIncorrect or missing prompt for text";
  ASSERT_THAT(actual_output, HasSubstr(NUMBER_PROMPT))
      << "\nIncorrect or missing prompt for rotation number";
  ASSERT_THAT(actual_output, HasSubstr(expected_enc))
      << "\nIncorrect encryption";
}
